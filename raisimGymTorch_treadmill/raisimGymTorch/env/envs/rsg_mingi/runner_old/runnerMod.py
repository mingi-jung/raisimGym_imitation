...
from raisimGymTorch.env.bin import rsg_mingi_20201111
...
env = VecEnv(impl=rsg_mingi_20201111.RaisimGymEnv(task_path + "/fullbody_k01",
...
if args.mode == "train" or args.mode == "retrain":
    ...
    saver = ConfigurationSaver(log_dir=home_path + "/raisimGymTorch/data/runnerlog",
    ...
    for update in range(total_update):
        ...
        if update % cfg['environment']['eval_every_n'] == 0:
            ...
            if not extraInfoNames:  # it the list is empty
        ...
        for step in range(n_steps):
            ...
            for i in range(env.num_envs):
                rewards_ep[i].append(rewards[i])
                for j in range(len(extraInfoNames)):
                    extrainfos_ep[i][j].append(infos[i][extraInfoNames[j]])

            for ienv in range(env.num_envs):
                if dones[ienv] == 1:
                    survival_check[ienv] = 0

            if step == n_steps-1:
                for ienv in range(env.num_envs):
                    if survival_check[ienv] == 1:
                        eprew = sum(rewards_ep[i])
                        eplen = len(rewards_ep[i])
                        epinfo = {"reward": eprew, "length": eplen}
                        infos[i]['episode'] = epinfo   # update infos
        ...
        actor.distribution.enforce_minimum_std((torch.ones(6)*0.2).to('cuda'))

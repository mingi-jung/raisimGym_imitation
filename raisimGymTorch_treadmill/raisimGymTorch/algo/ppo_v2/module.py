...
class Actor(nn.Module):
    def __init__(self, archi_shape, archi_actionvation_fn, archi_input_size, archi_output_size, dist_init_std, device='cpu'):
        ...
        modules = [nn.Linear(archi_input_size, archi_shape[0]), self.activation_fn()]
        scale = [np.sqrt(2)]

        for idx in range(len(archi_shape)-1):
            modules.append(nn.Linear(archi_shape[idx], archi_shape[idx+1]))
            modules.append(self.activation_fn())
            scale.append(np.sqrt(2))

        modules.append(nn.Linear(archi_shape[-1], archi_output_size))
        self.architecture = nn.Sequential(*modules)
        scale.append(np.sqrt(2))

        # self.init_weights(self.architecture, scale)
        [torch.nn.init.orthogonal_(module.weight, gain=scale[idx]) for idx, module in
            enumerate(mod for mod in self.architecture if isinstance(mod, nn.Linear))]
    ...
    def sample(self, obs):
        ...
        distribution = Normal(logits, self.dist_std.reshape(self.dist_dim))
    ...
    def parameters(self):
        return [*self.architecture.parameters(), self.dist_std]

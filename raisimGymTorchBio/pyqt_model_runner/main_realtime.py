...
class App(QWidget):
    ...
    def on_click_run_raisim(self):
        ...
        for step in range(max_steps):
            ...
            if self.extra_info1 != "":
                self.x_axis1.append(step*0.01)
                self.y_axis1.append(infos[0][self.extra_info1])
                if step > 200:
                    self.x_axis1.pop(0)
                    self.y_axis1.pop(0)
                self.graph_draw.plot(x=self.x_axis1, y=self.y_axis1)
                # self.graph_draw.plot().setData(x=self.x_axis1, y=self.y_axis1)
                # pg.QtGui.QApplication.processEvents()

            if self.extra_info2 != "":
                self.x_axis2.append(step*0.01)
                self.y_axis2.append(infos[0][self.extra_info2])
                if step > 200:
                    self.x_axis2.pop(0)
                    self.y_axis2.pop(0)
                self.graph_draw.plot(x=self.x_axis2, y=self.y_axis2, pen=pg.mkPen('r'))
                # pg.QtGui.QApplication.processEvents()

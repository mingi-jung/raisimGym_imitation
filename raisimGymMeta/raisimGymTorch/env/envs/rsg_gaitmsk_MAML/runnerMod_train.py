...
from raisimGymTorch.env.bin import rsg_gaitmsk_MAML as rsg_anymal
...
parser.add_argument('-wd1', '--disc1_weight', help='pre-trained discriminator1 weight path', type=str, default='')
parser.add_argument('-wd2', '--disc2_weight', help='pre-trained discriminator2 weight path', type=str, default='')
parser.add_argument('-wd3', '--disc3_weight', help='pre-trained discriminator3 weight path', type=str, default='')
...
disc1_weight_path = args.disc1_weight
disc2_weight_path = args.disc2_weight
disc3_weight_path = args.disc3_weight
...
ppo = PPO.PPO(actor=actor,
              ...
              log_intervals=10)
#print('error check 5')
disc1 = AMPDisc.AMPDisc(ndim=ob_amp_dim*2, num_envs=cfg['environment']['num_envs'], device=device, learning_rate=1e-5, betas=(0.5, 0.999))
disc2 = AMPDisc.AMPDisc(ndim=ob_amp_dim*2, num_envs=cfg['environment']['num_envs'], device=device, learning_rate=1e-5, betas=(0.5, 0.999))
disc3 = AMPDisc.AMPDisc(ndim=ob_amp_dim*2, num_envs=cfg['environment']['num_envs'], device=device, learning_rate=1e-5, betas=(0.5, 0.999))
# disc.load_refmotion_data(task_path + "/rsc/refmotion/" + cfg['environment']['expert_motion_sample'])
disc1.load_refmotion_data(home_path + "/raisimGymTorch/env/envs/rsg_gaitmsk_MAML/rsc/expert/" + cfg['environment']['expert_motion_sample_1'])
disc2.load_refmotion_data(home_path + "/raisimGymTorch/env/envs/rsg_gaitmsk_MAML/rsc/expert/" + cfg['environment']['expert_motion_sample_2'])
disc3.load_refmotion_data(home_path + "/raisimGymTorch/env/envs/rsg_gaitmsk_MAML/rsc/expert/" + cfg['environment']['expert_motion_sample_3'])
#print('error check 6')
if mode == 'retrain':
    if ppo_weight_path != '':
        load_param(ppo_weight_path, env, actor, critic, ppo.optimizer, saver.data_dir)
    if disc1_weight_path != '':
        disc1.load_state(disc1_weight_path)
    if disc2_weight_path != '':
        disc2.load_state(disc2_weight_path)
    if disc3_weight_path != '':
        disc3.load_state(disc3_weight_path)
...
d_loss_agent_1 = 0.0
d_loss_expert_1 = 0.0
d_gradient_penalty_1 = 0.0
d_loss_2 = 0.0
d_loss_agent_2 = 0.0
d_loss_expert_2 = 0.0
d_gradient_penalty_2 = 0.0
d_loss_3 = 0.0
d_loss_agent_3 = 0.0
d_loss_expert_3 = 0.0
d_gradient_penalty_3 = 0.0
...
for update in range(n_update):
    ...
    if update % cfg['environment']['eval_every_n'] == 0:
        ...
        disc1.save_state(os.path.abspath(saver.data_dir + "/full_disc1_" + str(update) + '.pt'))
        disc2.save_state(os.path.abspath(saver.data_dir + "/full_disc2_" + str(update) + '.pt'))
        disc3.save_state(os.path.abspath(saver.data_dir + "/full_disc3_" + str(update) + '.pt'))
    ...
    for step in range(n_steps):
        ...
        action = ppo1.observe(obs)
        rewards_task, dones, infos = env.step(action)
        obs_amp_1 = env.observe_amp()
        disc_score = disc1.observe(obs_amp_0, obs_amp_1)
        ...
        rewards = 0.11*rewards_task + 0.89*rewards_style
        # rewards = rewards_task

        # collect roll-out information per step for ppo update
        ppo1.step1(value_obs=obs, rews=rewards, dones=dones, infos=infos)
        done_sum = done_sum + sum(dones)
        reward_sum = reward_sum + sum(rewards)

    ppo1.update1(actor_obs=obs, value_obs=obs, log_this_iteration=update % 10 == 0, update=update)
    ...
    for step in range(n_steps):
        obs = env.observe()  # observation normalization is on
        obs_amp_0 = env.observe_amp()
        action = ppo2.observe(obs)
        rewards_task, dones, infos = env.step(action)
        obs_amp_1 = env.observe_amp()
        disc_score = disc2.observe(obs_amp_0, obs_amp_1)
        # rewards_style = np.exp(-1.0 * 0.7*(1-disc_score)**2)
        rewards_style = np.maximum(0, 1 - 0.25 * (disc_score - 1)**2)
        # print(rewards_style)
        ...
        # 20210804:1806, Okay now we train AMP
        rewards = 0.12*rewards_task + 0.88*rewards_style
        # rewards = rewards_task

        # collect roll-out information per step for ppo update
        ppo2.step2(value_obs=obs, rews=rewards, dones=dones, infos=infos)
        done_sum = done_sum + sum(dones)
        reward_sum = reward_sum + sum(rewards)

    ppo2.update2(actor_obs=obs, value_obs=obs, log_this_iteration=update % 10 == 0, update=update)
    ...
    ampinfos_ep = [[[] for _ in range(16)] for _ in range(env.num_envs)]
    ...
    for step in range(n_steps):
        obs = env.observe()  # observation normalization is on
        obs_amp_0 = env.observe_amp()
        action = ppo3.observe(obs)
        rewards_task, dones, infos = env.step(action)
        obs_amp_1 = env.observe_amp()
        disc_score = disc3.observe(obs_amp_0, obs_amp_1)
        # rewards_style = np.exp(-1.0 * 0.7*(1-disc_score)**2)
        rewards_style = np.maximum(0, 1 - 0.25 * (disc_score - 1)**2)
        # print(rewards_style)
        ...
        for ienv in range(env.num_envs):
            ...
            ampinfos_ep[ienv][3].append(d_loss_1)
            ampinfos_ep[ienv][4].append(d_loss_agent_1)
            ampinfos_ep[ienv][5].append(d_loss_expert_1)
            ampinfos_ep[ienv][6].append(d_gradient_penalty_1)
            ampinfos_ep[ienv][7].append(d_loss_2)
            ampinfos_ep[ienv][8].append(d_loss_agent_2)
            ampinfos_ep[ienv][9].append(d_loss_expert_2)
            ampinfos_ep[ienv][10].append(d_gradient_penalty_2)
            ampinfos_ep[ienv][11].append(d_loss_3)
            ampinfos_ep[ienv][12].append(d_loss_agent_3)
            ampinfos_ep[ienv][13].append(d_loss_expert_3)
            ampinfos_ep[ienv][14].append(d_gradient_penalty_3)
            ampinfos_ep[ienv][15].append(rewards_style_raw[ienv])
        ...
        for ienv in range(env.num_envs):
            if dones[ienv] == 1:
                ...
                extrainfo['disc_loss_1'] = sum(ampinfos_ep[ienv][3]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_agent_1'] = sum(ampinfos_ep[ienv][4]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_expert_1'] = sum(ampinfos_ep[ienv][5]) / infos[ienv]['episode']['length']
                extrainfo['disc_gradient_penalty_1'] = sum(ampinfos_ep[ienv][6]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_2'] = sum(ampinfos_ep[ienv][7]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_agent_2'] = sum(ampinfos_ep[ienv][8]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_expert_2'] = sum(ampinfos_ep[ienv][9]) / infos[ienv]['episode']['length']
                extrainfo['disc_gradient_penalty_2'] = sum(ampinfos_ep[ienv][10]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_3'] = sum(ampinfos_ep[ienv][11]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_agent_3'] = sum(ampinfos_ep[ienv][12]) / infos[ienv]['episode']['length']
                extrainfo['disc_loss_expert_3'] = sum(ampinfos_ep[ienv][13]) / infos[ienv]['episode']['length']
                extrainfo['disc_gradient_penalty_3'] = sum(ampinfos_ep[ienv][14]) / infos[ienv]['episode']['length']
                extrainfo['reward_style_raw'] = sum(ampinfos_ep[ienv][15]) / infos[ienv]['episode']['length']
                ...
                ampinfos_ep[ienv][8].clear()
                ampinfos_ep[ienv][9].clear()
                ampinfos_ep[ienv][10].clear()
                ampinfos_ep[ienv][11].clear()
                ampinfos_ep[ienv][12].clear()
                ampinfos_ep[ienv][13].clear()
                ampinfos_ep[ienv][14].clear()
                ampinfos_ep[ienv][15].clear()
        ...
        if step == n_steps - 1:   # in case of the last step
            for ienv in range(env.num_envs):
                ...
                extrainfo['disc_loss_1'] = sum(ampinfos_ep[ienv][3]) / n_steps
                extrainfo['disc_loss_agent_1'] = sum(ampinfos_ep[ienv][4]) / n_steps
                extrainfo['disc_loss_expert_1'] = sum(ampinfos_ep[ienv][5]) / n_steps
                extrainfo['disc_gradient_penalty_1'] = sum(ampinfos_ep[ienv][6]) / n_steps
                extrainfo['disc_loss_2'] = sum(ampinfos_ep[ienv][7]) / n_steps
                extrainfo['disc_loss_agent_2'] = sum(ampinfos_ep[ienv][8]) / n_steps
                extrainfo['disc_loss_expert_2'] = sum(ampinfos_ep[ienv][9]) / n_steps
                extrainfo['disc_gradient_penalty_2'] = sum(ampinfos_ep[ienv][10]) / n_steps
                extrainfo['disc_loss_3'] = sum(ampinfos_ep[ienv][11]) / n_steps
                extrainfo['disc_loss_agent_3'] = sum(ampinfos_ep[ienv][12]) / n_steps
                extrainfo['disc_loss_expert_3'] = sum(ampinfos_ep[ienv][13]) / n_steps
                extrainfo['disc_gradient_penalty_3'] = sum(ampinfos_ep[ienv][14]) / n_steps
                extrainfo['reward_style_raw'] = sum(ampinfos_ep[ienv][15]) / n_steps
                infos[ienv]['extrainfo'] = extrainfo  # update extrainfo infos

        # collect roll-out information per step for ppo update
        ppo3.step3(value_obs=obs, rews=rewards, dones=dones, infos=infos)
        done_sum = done_sum + sum(dones)
        reward_sum = reward_sum + sum(rewards)

    ppo3.update3(actor_obs=obs, value_obs=obs, log_this_iteration=update % 10 == 0, update=update)
    ...
    for step in range(n_steps):
        ...
        # collect roll-out information per step for ppo update
        ppo.step1(value_obs=obs, rews=rewards, dones=dones, infos=infos)
        done_sum = done_sum + sum(dones)
        reward_sum = reward_sum + sum(rewards)
    ...
    for step in range(n_steps):
        ...
        # collect roll-out information per step for ppo update
        ppo.step2(value_obs=obs, rews=rewards, dones=dones, infos=infos)
        done_sum = done_sum + sum(dones)
        reward_sum = reward_sum + sum(rewards)
    ...
    for step in range(n_steps):
        ...
        # collect roll-out information per step for ppo update
        ppo.step3(value_obs=obs, rews=rewards, dones=dones, infos=infos)
        done_sum = done_sum + sum(dones)
        reward_sum = reward_sum + sum(rewards)
    ...
    obs = env.observe()  # observation normalization is on
    ppo.update1(actor_obs=obs, value_obs=obs, log_this_iteration=update % 10 == 0, update=update)
    ppo.update2(actor_obs=obs, value_obs=obs, log_this_iteration=update % 10 == 0, update=update)
    ppo.update3(actor_obs=obs, value_obs=obs, log_this_iteration=update % 10 == 0, update=update)

    # slow down the disc training
    if update % 1000 == 0:
        disc_update_skip = disc_update_skip + 1
    # if update % disc_update_skip == 0 and update < 10000:
    #     d_loss, d_loss_agent, d_loss_expert, d_gradient_penalty = disc.update()
    if update % disc_update_skip == 0:
        d_loss_1, d_loss_agent_1, d_loss_expert_1, d_gradient_penalty_1 = disc1.update()
        d_loss_2, d_loss_agent_2, d_loss_expert_2, d_gradient_penalty_2 = disc2.update()
        d_loss_3, d_loss_agent_3, d_loss_expert_3, d_gradient_penalty_3 = disc3.update()
    disc1.agent_data.clear()
    disc2.agent_data.clear()
    disc3.agent_data.clear()

...
class App(QWidget):
    ...
    def initUI(self):
        ...
        ## mingi reference motions
        label4 = QLabel('reference motion:', self)
        label4.move(100, 455)

        # self.reference_name= "/home/opensim2020/raisim_v5_workspace/raisimLib/subject_exp/subject06/subject06_AMP/subject06_walking13_straight02.txt"
        self.reference_name= "/home/opensim2020/raisim_v6_workspace/raisimLib/raisimGymTorch_treadmill/raisimGymTorch/env/envs/rsg_mingi/rsc/motions/subject06/subject06_walking13_straight03_cyclic.txt"
        lineedit_controller_filepath = QLineEdit(self.reference_name, self)
        lineedit_controller_filepath.move(100, 470)
        lineedit_controller_filepath.textChanged.connect(self.reference_name_update)

        self.urdf_name= "/home/opensim2020/raisim_v6_workspace/raisimLib/examples/src/server/full_body/subject06_scaling.urdf"
        lineedit_controller_filepath = QLineEdit(self.urdf_name, self)
        lineedit_controller_filepath.move(100, 490)
        lineedit_controller_filepath.textChanged.connect(self.urdf_name_update)

        self.map_name= "/home/opensim2020/raisim_v6_workspace/raisimLib/examples/src/server/KAIST_terrain/terrain_stairs_ascend.png"
        lineedit_controller_filepath = QLineEdit(self.map_name, self)
        lineedit_controller_filepath.move(100, 510)
        lineedit_controller_filepath.textChanged.connect(self.map_name_update)

        button_ref = QPushButton('Run ref.', self)
        button_ref.setToolTip('It runs reference motion')
        button_ref.move(100, 530)
        button_ref.clicked.connect(self.on_click_run_ref)
    ...
    def reference_name_update(self, text):
        self.reference_name = text

    def urdf_name_update(self, text):
        self.urdf_name = text

    def map_name_update(self, text):
        self.map_name = text
    ...
    def on_click_run_ref(self):

        raisim.World.setLicenseFile(os.path.dirname(os.path.abspath(__file__)) + "/../../rsc/activation.raisim")
        with open(self.reference_name, 'r') as f:
            json_data = json.load(f)

        world = raisim.World()
        world.setTimeStep(0.001)
        server = raisim.RaisimServer(world)
        ground = world.addHeightMap(self.map_name, 28.2, 0, 80, 80, 0.00027, -13.24)
        # ground = world.addHeightMap(self.urdf_name)
        anymal = world.addArticulatedSystem(self.urdf_name)
        anymal.setName("anymal")
        server.launchServer(8080)

        jointIdxConvTable = [0, 2, 1, 3, 4, 5, 6, 7, 8, 9, 10, 21, 22, 23, 24, 25, 26, 27, 28, 11, 12, 13, 14, 15, 29, 30, 31, 32, 33, 34, 35, 36, 16, 17, 18, 19, 20]
        for k in range(3):
            for i in range(len(json_data['Frames'])):
                self.gc_ref = np.zeros(37)

                for j in range(len(self.gc_ref)):
                    self.gc_ref[int(jointIdxConvTable[j])] = json_data['Frames'][i][j+1]
                    # print(json_data['Frames'][i][j+1])
                    # print(self.gc_ref[int(jointIdxConvTable[j])])
                    # print(jointIdxConvTable[j])

                    qw1 = self.gc_ref[3]
                    qv1 = np.array([self.gc_ref[4], self.gc_ref[5], self.gc_ref[6]])
                    qw2 = 0.7071068
                    qv2 = np.array([0.7071068, 0, 0])
                    qw3 = 0
                    qv3 = np.array([0, 0, 0])

                    qw3 = qw2*qw1 - np.dot(qv1,qv2)
                    qv3 = qw2*qv1 + qw1*qv2 + np.cross(qv2,qv1)
                ...
                # print(self.gc_ref)
                anymal.setGeneralizedCoordinate(self.gc_ref)
                time.sleep(0.1)

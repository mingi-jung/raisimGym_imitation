...
public:
    ...
    void SetScale(const std::vector<std::string> &scale_link, const std::vector<double> &scale_value, const std::vector<raisim::Vec<3>> &trs, const std::vector<raisim::Mat<3, 3>> &rot, double l0);
...
void MuscleModel::Update(const std::vector<raisim::Vec<3>> &trs, const std::vector<raisim::Mat<3, 3>> &rot, std::vector<double> &ForceTable) {
    ...
    if (HillTypeOn == 1) {
        ...
        if (Activation < 0.0)
            ...
            Activation = 1.0;
        Ff_cos = Activation * Fiso; // it should be checked
    }
    CalcMomentArm();
...
void MuscleModel::SetScale(double scale) {
    TendonSlackLength *= scale;
    OptFiberLength *= scale;
}

void MuscleModel::SetScale(const std::vector<std::string> &scale_link, const std::vector<double> &scale_value, const std::vector<raisim::Vec<3>> &trs, const std::vector<raisim::Mat<3, 3>> &rot, double l0) {
    /// pre-scaled length

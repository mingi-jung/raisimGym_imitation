...
from raisimGymTorch.env.bin import rsg_gaitmsk_amp
...
env = VecEnv(impl=rsg_gaitmsk_amp.RaisimGymEnv(task_path + "/rsc",
...
expert_data1 = expert_data1[1:2501]
expert_data2 = expert_data2[1:2501]
expert_data3 = expert_data3[1:2501]
expert_data4 = expert_data4[1:2501]

expert_data = expert_data1 + expert_data2 + expert_data3 + expert_data4;

print(len(expert_data))

with open("agent_all_new.json", "w") as new_file:
    json.dump(expert_data, new_file, indent="\t")

# with open('AMP_subject06_01.json') as f:
#     json_data5 = json.load(f)
#
# expert_data5 = json_data5['expert']
# expert_data5 = torch.tensor(expert_data5)
...
expert_data = expert_data1

obs_amp = env.observe_amp()
obs_amp = torch.tensor(obs_amp)
obs_amp_all = torch.cat([obs_amp])
...
for update in range(n_update):
    ...
    for step in range(n_steps):
        ...
        if step%10 == 0:
            if len(obs_amp_all) < 100000:
                obs_amp_all = torch.cat([obs_amp_all, obs_amp])
            else:
                obs_amp_all = torch.cat([obs_amp_all[200:], obs_amp])



        # reward_style = 0.25 * max(0, 1 - 0.25 * (torch.mean(D(obs_amp)) - 1)**2)
        # # print(rewards)
        # # print(reward_style)
        # reward_style_double = reward_style.item()
        # # print(reward_style_double)
        # rewards = rewards + reward_style_double
        # # print(rewards)

        for ienv in range(env.num_envs):
            reward_style = 0.3 * max(0, 1 - 0.25 * (D(obs_amp[ienv]) - 1)**2)
    ...
    N_agent = len(obs_amp_all) - 1
    k_agent = 200
    indices_agent = torch.tensor(random.sample(range(N_agent), k_agent))
    indices_agent = torch.tensor(indices_agent)
    sampled_agent_data = obs_amp_all[indices_agent]


    D_real = D(sampled_expert_data)
    D_fake = D(sampled_agent_data)




    D_loss = (torch.mean((D_real - 1)**2) + torch.mean((D_fake + 1)**2))
    writer.add_scalar("D_loss", D_loss, update)
    ...
    writer.add_scalar("average_performance", average_performance, update)

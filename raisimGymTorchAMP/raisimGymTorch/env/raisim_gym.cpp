...
PYBIND11_MODULE(RAISIMGYM_TORCH_ENV_NAME, m) {
  py::class_<VectorizedEnvironment<ENVIRONMENT>>(m, RSG_MAKE_STR(ENVIRONMENT_NAME))
    ...
    .def("observe_amp", &VectorizedEnvironment<ENVIRONMENT>::observe_amp)
    ...
    .def("getObDim_amp", &VectorizedEnvironment<ENVIRONMENT>::getObDim_amp)
    ...
    .def("getExtraInfoNames", &VectorizedEnvironment<ENVIRONMENT>::getExtraInfoNames)  // by skoo
    .def("getExtraInfoDim", &VectorizedEnvironment<ENVIRONMENT>::getExtraInfoDim);   // by skoo
}

...
class RaisimGymVecEnv:
    ...
    def __init__(self, impl, cfg, normalize_ob=True, seed=0, normalize_rew=True, clip_obs=10.):
        ...
        self.num_obs_amp = self.wrapper.getObDim_amp()
        self.num_acts = self.wrapper.getActionDim()

        self._observation = np.zeros([self.num_envs, self.num_obs], dtype=np.float32)
        self._observation_amp = np.zeros([self.num_envs, self.num_obs_amp], dtype=np.float32)
    ...
    def load_scaling(self, dir_name, iteration):
        ...
        count_file_name = dir_name + "/count" + str(iteration) + ".csv"
        temp_mean = np.loadtxt(mean_file_name, dtype=np.float32)
        temp_var = np.loadtxt(var_file_name, dtype=np.float32)
        temp_count = np.loadtxt(count_file_name, dtype=np.float32)
        self.obs_rms.mean = np.tile(temp_mean, (self.num_envs, 1))
        self.obs_rms.var = np.tile(temp_var, (self.num_envs, 1))
        self.obs_rms.count = float(temp_count)
    ...
    def save_scaling(self, dir_name, iteration):
        ...
        np.savetxt(count_file_name, np.array([self.obs_rms.count]))
